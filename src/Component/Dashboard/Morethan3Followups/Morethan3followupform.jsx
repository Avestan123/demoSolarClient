...
const Morethan3followupform = () => {
  ...
  useEffect(() => {
    setValue("clientName", row?.lead?.clientName);
    setValue("number", row?.lead?.number);
    setValue("email", row?.lead?.email);
    setValue("address", row?.lead?.address);

    setValue("state", row?.lead?.state);
    setstate(row?.lead?.state);
    setValue("city", row?.lead?.city);
    setcity(row?.lead?.city);

    setValue("pincode", row?.lead?.pincode);
    setValue("source", row?.lead?.source);
    setValue("project", row?.lead?.project);
    setValue("projectKeyword", row?.lead?.projectKeyword);
    setValue("retail", row?.lead?.retail);
    setValue("retailName", row?.lead?.retailName);
    setValue("retailNumber", row?.lead?.retailNumber);
    setValue("retailEmail", row?.lead?.retailEmail);
    setValue("retailAddress", row?.lead?.retailAddress);
    setValue("retailCity", row?.lead?.retailCity);
    setValue("retailState", row?.lead?.retailState);
    setValue("retailPincode", row?.lead?.retailPincode);
    setValue("retailSource", row?.lead?.retailSource);
    setValue("retailProject", row?.lead?.retailProject);
    setValue("dob", row?.lead?.dob);
    setValue("requirement", row?.lead?.requirement);
    setValue("kilowatts", row?.lead?.kilowatts);
    setValue("clientlevel", row?.lead?.clientlevel);
    setValue("followUpDate", row?.lead?.followUpDate);
    setValue("remarks", row?.lead?.remarks);
    setValue("referralName", row?.lead?.referralName);

    setselectedFiles((prevfile) => ({
      ...prevfile,
      adharcard: row?.lead?.adharcard,
    }));
    console.log("adhar card", row?.adharcard);
    setselectedFiles((prevfile) => ({
      ...prevfile,
      pancard: row?.lead?.pancard,
    }));
    setselectedFiles((prevfile) => ({
      ...prevfile,
      electricitybill: row?.lead?.electricitybill,
    }));
    setselectedFiles((prevfile) => ({
      ...prevfile,
      taxreceipt: row?.lead?.taxreceipt,
    }));
  ...
  return (
    <>
      ...
      >
        ...
        {console.log(row.lead.additionalFollowups)}
        <Center>
          <Text ml="1rem" mt="1rem" fontSize="1.4rem" fontWeight="bold">
            {" "}
            Follow-up History
          </Text>
        </Center>
        {row?.lead?.additionalFollowups.map((item) => (
          <Stack
...
export default Morethan3followupform;
