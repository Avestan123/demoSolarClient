...
function EmployeeTable() {
  ...
  const salesmanager =
    role === "Sales Manager" || role === "General Sales Manager";
  ...
  const columns = [
    {
      ...
      selector: (row) => {
        ...
        console.log("From New Employee role", row);
      },
      sortable: true,
      cell: (row) => {
        if (editMode === row?.employee?.employeeId) {
          return (
            <Input
              type="text"
              value={row?.employee?.name}
              onChange={(e) => handleEditChange(row, "name", e.target.value)}
            />
          );
        } else {
...

    {
      ...
      sortable: true,
      cell: (row) => {
        if (editMode === row?.employee?.email) {
          return (
            <Input
              type="email"
...

    {
      ...
      cell: (row) => {
        return (
          <>
            {editMode === row?.employee?.employeeId ? (
              <Input
                id={`edit-input-${row?.employee?.employeeId}`}
                type="text"
    ...
    {
      name: "Password",
      selector: (row) => row?.employee?.employeePassword,
      sortable: true,
      display: isAdmin,
      cell: (row) => {
        if (editMode === row?.employee?.employeeId) {
          return (
            <Input
              type="text"
              value={row?.employee?.employeePassword}
              onChange={(e) =>
          ...
          return row?.employee?.employeePassword;
        }
      },
    },
    // : "",
    ...
    {
      name: "Role",
      selector: (row) => row?.employee?.employeeRole,
      sortable: true,
      ...
      cell: (row) => {
        if (editMode === row?.employee?.employeeId) {
          return (
            <Input
              type="text"
              value={row?.employee?.employeeRole}
              onChange={(e) =>
          ...
          return row?.employee?.employeeRole;
        }
      },
    },
    {
      name: "Total FollowUps",
      selector: (row) => row?.totalFollowUps,
      sortable: true,
      show: salesmanager,
      cell: (row) => {
        if (editMode === row?.totalFollowUps) {
          return (
            <Input
              type="text"
              value={row?.totalFollowUps}
              onChange={(e) =>
                handleEditChange(row, "employeeRole", e.target.value)
              }
            />
  ...
  return loading ? (
    ...
    <Center>
      <Box width={{ base: "90vw", md: "70vw" }} overflowX="auto" p={4}>
        {console.log("Filter setdata", filter)}
        <Center>
